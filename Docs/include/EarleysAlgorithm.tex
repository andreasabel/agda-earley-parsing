...
\chapter{Earley's Algorithm} \label{Earleys}
	...
	\section{Description}
		...
		a new item.
                \todo{Rephrase.  This sentence has grammatical
                  problems, and funny uses of ``it''.}
		...
		$X$ is the non-terminal this item represents a partial
                parse of,
\todo{Problematic dangling proposition.}
...
\todo{The letter $t$ could be taken for an element of $T$, a
  terminal.  When possible, use $w$ instead.  In this case `input'
  felt suitable.}
		\begin{code}
		...
		\end{code}
\todo{There is a notation involving ←∷ which has not been explained.
I first thought it is a different judgement.
Extra parentheses also do not hurt in parsing complicated judgements.}

		Here, a value with type $G \cdot \mathit{input} \vdash u / v \rightarrow^* X /
		...
		$G$.
\todo{This explanation is too brief/condensed.  Do a one-to-one
  matching of the elements of the judgement
  $\earley{X}{\alpha}{\beta}{i} \in \mathbb{S}_j$ and its
  formalization in Agda.
You should also explain the invariants of this judgement, like $u$ is
a suffix of `input' and $v$ is a suffix of $u$.  The difference
between $u$ and $v$ is what is parsed by $\alpha$. Any more?}
