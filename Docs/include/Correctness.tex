...
\chapter{Correctness proofs} \label{Correctness}
	...
	\section{Soundness}
		...
		\begin{code}
			...
			sound-scanr : ∀ {a w v} -> (ω : ESet w (a ∷ v)) ->
		...
		the correct item set to use from a \codett{ESet}, its soundness is
		...
		\begin{code}
			sound-pred-comp₁ : ∀ {w v} (ω : ESet w v) -> ∀ ss rs ->
			  (∀ {i} -> i ∈ ss -> Valid i) ->
			  (∀ {i} -> i ∈ rs -> Valid i) ->
		...
		\begin{code}
			sound-pred-comp₂ : ∀ {w v} (ω : ESet w v) -> ∀ ss rs m p q ->
			  (∀ {i} -> i ∈ ss -> Valid i) ->
			  (∀ {i} -> i ∈ rs -> Valid i) ->
		...
		\begin{code}
			sound-pred-comp : ∀ {w v} {ω : ESet w v} ->
			  Sound ω -> Sound (pred-comp ω)

			sound-step₀ : ∀ {w a v} {ω : ESet w (a ∷ v)} ->
			  Sound ω -> Sound (step₀ ω)

			sound-parse₀ : ∀ {w v} -> {ω : ESet w v} ->
	...
	\section{Completeness}
		...
		completeness for \codett{ESet}s:

		\begin{code}
			mutual
			  Complete : ∀ {v w} -> ESet w v -> Set
		...
		Here, the \codett{Complete*} property for a \codett{ESet} would mean
		that all item sets, except for the very topmost one, are complete, and
		\codett{Complete} represents completeness for \codett{ESet}s: all
