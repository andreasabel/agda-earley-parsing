...
\todo{Rewrite sentence. Avoid parenthesized remarks!
        Double parenthesization is a no-go!}
	...
	of $O(n^3)$ only in the worst case,\todo{Briefly characterize
          the worst case.}
	...
	parse more grammars than linear parsers,\todo{``Linear
          parsers'' has not been defined/mentioned befor.}
        while being faster than algorithm
	such as CYK or Valiant's. \todo{Say something about
          verification here for a smooth transition to the next topic: Agda.}
