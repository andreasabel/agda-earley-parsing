...
\chapter{Agda} \label{Agda}
	...
	\section{Baby steps}
		...
		\codett{suc} representing the successor function. Let us take a look at
...
%
	...
	\section{Types as propositions}
		...
		natural numbers. The first constructor, \codett{≤₀}, takes one natural
		number \codett{n} (as an implicit argument), and constructs a value of
		the type \codett{zero ≤ n}, which means that the first argument to the
		type must always be \codett{zero} when \codett{≤₀} is used. \codett{≤ₛ}
		...
		of type \codett{suc zero ≤ zero}: The constructor \codett{≤₀} cannot be
		...
		we would need \codett{suc zero}), and neither can \codett{≤ₛ}, because
		the second type arguments do not match (no matter the arguments, the
		second type argument of the result will be \codett{suc}
		\emph{something}, which is not equal to \codett{zero}). In fact, as the
	...
	\section{Equality and normalization}
		...
		The type \codett{\_≡\_} takes three arguments, a type \codett{T}, and
		two values of that type. Since the constructor can only construct
		values which have the same value in both type arguments, \codett{\_≡\_}
		...
		matching on \codett{refl} will convince the compiler that the terms are
		...
		\end{code}
	 	...
	 	the lists \codett{as}, \codett{bs} and \codett{cs} look like, so we do
	 	not know what is going to happen when \codett{\_++\_} is applied to
	 	them.  However, when we pattern-match on \codett{as} this changes. In
	 	the first case, with \codett{as} $=\epsilon$, when normalizing the term
